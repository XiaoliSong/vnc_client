...
if(get_query_string("way")=="view"){
	document.title="请求查看 "+get_query_string("to_see_id")+" 的桌面";
}
else{
	document.title="请求控制 "+get_query_string("to_see_id")+" 的桌面";
}
...
jsmpeg.prototype.initSocketClient = function( client ) {
	...
	var pw=get_query_string("pw");
	var to_see_id=get_query_string("to_see_id");
	var pw=get_query_string("pw");
	var way=get_query_string("way");
	this.client.send(id+";"+pw+";"+to_see_id+";"+way);
...
jsmpeg.prototype.receiveSocketMessage = function( event ) {
	if(this.count==0){
		if(event.data=="error_login"){
			alert("账号和密码不对");
			console.log("wrong ip / pw");
			document.title="帐号和密码不匹配";
			this.client.close();
		}
		if(event.data=="error_access"){
			alert("没有权限");
			console.log("error_access");
			document.title="没有权限获取 "+get_query_string("to_see_id")+" 的桌面";
			this.client.close();
		}
		...
		if(event.data=="control"){
			allow_input=1;
			console.log("控制模式");
			document.title="正在控制 "+get_query_string("to_see_id")+" 的桌面";
		}
		else{
			document.title="正在查看 "+get_query_string("to_see_id")+" 的桌面";
		}
		this.count++;
		return;
	}
