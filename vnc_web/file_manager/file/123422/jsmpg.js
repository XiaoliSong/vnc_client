...
jsmpeg.prototype.receiveSocketMessage = function( event ) {
	if(this.count<2){
		if(this.count==0){
			var cur_url=window.location.href;//获取链接名称
			var ip=cur_url.substring(cur_url.indexOf("?IP=")+4,cur_url.indexOf("&PW="))
			var pw=cur_url.substring(cur_url.indexOf("&PW=")+4);
			this.client.send(ip+";"+pw);
		}
		else{
			if(event.data=="error"||event.data=="No Server"){
				alert("Wrong IP or PASSWORD!");
