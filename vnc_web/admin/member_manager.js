function add(){
	...
	var req_url="./member_add.php?id="+get_query_string("id")+"&pw="+get_query_string("pw")+"&add_id="+add_id+"&add_pw="+add_pw;
	var xmlhttp;
...
function delete_member(){
	...
	var req_url="./member_delete.php?id="+get_query_string("id")+"&pw="+get_query_string("pw")+"&delete_id="+delete_id;
	var xmlhttp;
	if (window.XMLHttpRequest){// code for IE7+, Firefox, Chrome, Opera, Safari
		xmlhttp=new XMLHttpRequest();
	}
...
function search(){
	...
	var req_url="./member_search.php?id="+get_query_string("id")+"&pw="+get_query_string("pw")+"&search_id="+search_id;
	var xmlhttp;
	if (window.XMLHttpRequest){// code for IE7+, Firefox, Chrome, Opera, Safari
		xmlhttp=new XMLHttpRequest();
	}
	...
	xmlhttp.onreadystatechange=function() {
		if (xmlhttp.readyState==4 && xmlhttp.status==200){
			...
			if(xmlhttp.responseText.length>2){
				...
				obj.innerHTML="存在该用户!<br />";
				obj.innerHTML+="用户名："+mem[0]["id"]+"<br />";
				obj.innerHTML+="注册时间："+mem[0]["reg_time"]+"<br />";
				obj.innerHTML+="简介："+mem[0]["intro"]+"<br />";
			}
			else{
				if(xmlhttp.responseText==0){
					obj.innerHTML="不存在该用户!";
				}
				else{
					obj.innerHTML="错误："+xmlhttp.responseText;
				}
